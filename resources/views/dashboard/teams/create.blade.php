...
@section('container')
<form method="post" action="/dashboard/teams" class="max-w-full" enctype="multipart/form-data">
    @csrf
    <div class="mb-5">
    ...
    <div class="mb-5">
        ...
        <input type="text" id="jabatan" name="jabatan" class="@error('jabatan') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{old('jabatan')}}"/>
        @error('jabatan')
        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
        @enderror
    </div>
...
<script>
    ...
    title.addEventListener('change', function(){
        fetch('/dashboard/teams/checkSlug?title=' + title.value)

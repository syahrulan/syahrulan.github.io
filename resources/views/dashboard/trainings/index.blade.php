...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    @if(session()->has('success'))
        ...
        @endif
    <a href="/dashboard/trainings/create"><button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"><i class="fa-solid fa-plus"></i> Create New Post</button></a>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        @foreach($trainings as $training)
            <tr class="bg-gray-100 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                {{  Str::limit($training ['title']) }}
                </th>
                <td class="p-4">
                    <img src="{{ asset( 'storage/' . $training->image) }}" class="w-16 md:w-32 max-w-full max-h-full" alt="Berita Terkini">
                </td>
                <td class="px-6 py-4">
                {{ Str::limit($training ['category']) }}
                </td>
                <td class="px-6 py-4">
                {{ Str::limit($training ['package']) }}
                </td>
                <td class="px-6 py-4">
                {{ Str::limit($training ['harga']) }}
                </td>
                <td class="px-6 py-4">
                {!! Str::limit(strip_tags($training->body), 50, '...') !!}
                </td>
                <td class="px-6 py-4">
